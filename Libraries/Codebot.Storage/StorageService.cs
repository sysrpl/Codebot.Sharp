...
{
	/// <summary>
	/// Describes how to connect to a particular Storage server.
	/// </summary>
	public class StorageService
	{
		...
		/// </summary>
		public event EventHandler<StorageProgressEventArgs> AddObjectProgress;

		/// <summary>
		/// Reports progress for any operation that gets an object from a bucket.
		/// </summary>
		public event EventHandler<StorageProgressEventArgs> GetObjectProgress;

		/// <summary>
		/// Fired before an StorageRequest operating against this service is authorized and sent
		/// out to the Storage server. This is a good opportunity to modify every StorageRequest 
		/// created internally by this class, for instance, to set the Proxy server or ServicePoint.
		/// </summary>
		public event EventHandler<StorageRequestArgs> BeforeAuthorize;

		/// <summary>
		/// Gets or sets the hostname of the Storage server, usually "Storage.amazonaws.com" unless you
		/// are using a 3rd party Storage implementation.
		/// </summary>
		public string Host { get; set; }

		/// <summary>
		/// Gets or sets the service provider, usually StorageProvider.Amazon unless you
		/// are using a 3rd party Storage implementation.
		/// </summary>
		...
		/// <summary>
		/// Gets or sets a custom port to use to connect to the Storage server. The default is zero, which
		...
		public string SecretAccessKey
		{
			...
			set
			{
				...
				authorizer = !string.IsNullOrEmpty(value) ? new StorageAuthorizer(this) : null;
			}
		}
		...
		/// <summary>
		/// Creates a new StorageService with the default values.
		/// </summary>
		public StorageService()
		{
			this.Provider = StorageProvider.Amazon;
			this.UseSsl = true;
			this.DefaultDelimiter = "/";
		}

		internal void AuthorizeRequest(StorageRequest request, HttpWebRequest webRequest, string bucketName)
		{
			if (BeforeAuthorize != null)
				BeforeAuthorize(this, new StorageRequestArgs(request));

			// if you haven't set a secret access key, we can't authorize anything! maybe
			// you're talking to a mock Storage server. At any rate, the server will complain
		...
		/// <summary>
		/// Queries Storage about the existance and ownership of the given bucket name.
		/// </summary>
		public BucketAccess QueryBucket(string bucketName)
		{
			...
			catch (StorageException exception)
			{
				...
				{
					case StorageErrorCode.NoSuchBucket: return BucketAccess.NoSuchBucket;
					case StorageErrorCode.AccessDenied: return BucketAccess.NotAccessible;
					default: throw;
				}
			}
		}
		...
		public bool ObjectExists(string bucketName, string key)
		{
			...
			// This is the recommended method from the Storage API docs.
			try
			{
		...
		/// <summary>
		/// Queries Storage to determine whether the given bucket resides in the Europe location.
		/// </summary>
				...
				using (var response = request.GetResponse())
				{
					...
					if (response.IsTruncated)
					{
						...
						// so we'll use the last ObjectEntry's key as the next delimiter.
							...
							throw new Exception("Storage Server is misbehaving.");
					}
					else
						break; // we're done!
				}
		...
		/// This method itself does not communicate with Storage and will return immediately.
		/// </summary>
		...
		/// <summary>
		/// Creates a pre-authorized URI valid for performing a GET on the given Storage object
		...
		/// into a raw "+", which is interpreted by Amazon Storage as a space (standard URI conventions).
		...
		/// <summary>
		/// Adds an object to Storage by reading the specified amount of data from the given stream.
		/// </summary>
		...
		/// <summary>
		/// Adds an object to Storage by reading all the data in the given stream. The stream must support
		/// the Length property.
		/// </summary>
		...
		/// <summary>
		/// Uploads the contents of an existing local file to Storage.
		/// </summary>
		...
		/// <summary>
		/// Uploads the contents of a string to Storage. This method is only appropriate for
		...
		/// <summary>
		/// Gets an existing object in Storage and copies its data to the given Stream.
		/// </summary>
		...
		/// <summary>
		/// Downloads an existing object in Storage to the given local file path.
		/// </summary>
		...
		/// <summary>
		/// Downloads an existing object in Storage and loads the entire contents into a string.
