...
{
    /// <summary>
    /// Gets an Storage object, partial object, or just object metadata.
    /// </summary>
    public class GetObjectRequest : StorageRequest<GetObjectResponse>
    {
        // store this to emulate the behavior of WebRequest.IfModifiedSince
        DateTime? ifUnmodifiedSince;
        bool rangeWasAdded;
        
        public GetObjectRequest(StorageService service, string bucketName, string key)
            : this(service, bucketName, key, false)
        {
        }

        public GetObjectRequest(StorageService service, string bucketName, string key, bool metadataOnly)
        ...
        {
            if (rangeWasAdded)
                throw new InvalidOperationException("Storage only supports a single range specifier.");
    ...
    /// <summary>
    /// Represents the Storage response containing the queried object's metadata and/or data.
    /// </summary>
    public sealed class GetObjectResponse : StorageResponse
    {
        ...
        protected override void ProcessResponse()
        {
            ...
            {
                if (key.StartsWith(StorageHeaders.MetadataPrefix))
                {
                    string trimmedKey = key.Substring(StorageHeaders.MetadataPrefix.Length);

                    foreach (string value in WebResponse.Headers.GetValues(key))
                        Metadata.Add(trimmedKey, value);
                }
                else if (key == StorageHeaders.MissingMetadata)
                {
                    MissingMetadataHeaders = int.Parse(WebResponse.Headers[StorageHeaders.MissingMetadata]);
                }
            }
        }
        ...
        /// Note that Codebot.Storage manages removing the special "x-amz-meta" header prefix for you.
        /// </summary>
