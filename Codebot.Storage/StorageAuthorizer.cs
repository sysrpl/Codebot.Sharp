...
{
    class StorageAuthorizer : IComparer<string>
    {
        StorageService service;
        HMACSHA1 signer;

        public StorageAuthorizer(StorageService service)
        {
        ...
        /// </summary>
        /// <remarks>
        /// See http://docs.amazonwebservices.com/AmazonStorage/2006-03-01/RESTAuthentication.html
        /// </remarks>
        public void AuthorizeRequest(HttpWebRequest request, string bucketName)
        {
            request.Headers[StorageHeaders.AmazonDate] = DateTime.UtcNow.ToString("r");
        ...
        void AppendCanonicalizedAmzHeaders(HttpWebRequest request, StringBuilder stringToSign)
        {
            ...
            foreach (string header in request.Headers)
                if (header.StartsWith(StorageHeaders.AmazonHeaderPrefix))
